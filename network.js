...
if(canvas){
  ...
  function draw(){
    ...
    for(let i=0;i<points.length;i++)for(let j=i+1;j<points.length;j++){
      ...
      if(d<150){ctx.strokeStyle=`rgba(213,166,66,${(1-d/150)*.18})`;ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.stroke();}
    }
    for(const p of points){
      const g=ctx.createRadialGradient(p.x,p.y,0,p.x,p.y,10);
      g.addColorStop(0,'rgba(243,217,140,.9)');g.addColorStop(1,'rgba(213,166,66,0)');
      ctx.fillStyle=g;ctx.beginPath();ctx.arc(p.x,p.y,10,0,Math.PI*2);ctx.fill();
    }
    frame=requestAnimationFrame(draw);
  }
  resize();draw();addEventListener('resize',()=>{cancelAnimationFrame(frame);resize();draw();});
}