const TRIGGERS = [
  ...
  { id: "location_request", label: "location request", weight: 70, words: ["coordinates", "coordinate", "send location", "share location", "location pin", "gps", "координат", "локацию", "геолокацию", "геопозицию", "местоположение", "пришли где ты", "скинь где ты"] },
...
function analyzeRisk(text) {
  ...
  const hasAction = matches.some(match => ["money", "code", "remote_access", "location_request", "personal_data"].includes(match.id));
